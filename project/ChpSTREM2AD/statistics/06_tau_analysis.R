...
for (pair_cfg in project_config$tau$linear_pairs) {
...
for (partial_pair in project_config$tau$partial_pairs) {
...
overall_path <- file.path(result_summary_dir, "tau_linear_overall.csv")
by_group_path <- file.path(result_summary_dir, "tau_linear_by_group.csv")
partial_overall_path <- file.path(result_summary_dir, "tau_partial_overall.csv")
partial_by_group_path <- file.path(result_summary_dir, "tau_partial_by_group.csv")
table_csv <- file.path(result_tables_dir, "table_tau_linear_primary.csv")
table_html <- file.path(result_tables_dir, "table_tau_linear_primary.html")
...
export_three_line_table(
  ...
  title = "Tau Association Analysis"
)

append_analysis_log(
  project_root = project_root,
  analysis_name = "06_tau_analysis",
  output_files = c(overall_path, by_group_path, partial_overall_path, partial_by_group_path, table_csv, table_html, plot_files),
  note = "Completed adjusted linear models, partial correlations, and scatter plots for Tau with ChPICV and sTREM2 in the current result version.",
